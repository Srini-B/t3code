...
it.effect("always grants pull-requests and gates browser and device access independently", () =>
  Effect.gen(function* () {
    ...
    expect(yield* capabilitiesOf(withPreview)).toEqual(["ask-user", "preview", "pull-requests"]);
    expect(yield* capabilitiesOf(withoutPreview)).toEqual(["ask-user", "pull-requests"]);
    expect(yield* capabilitiesOf(withDevice)).toEqual(["ask-user", "device", "pull-requests"]);
  }),
);
