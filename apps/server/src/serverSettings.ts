...
const PersistedOptionalProviderSettings = Schema.Struct({
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      amp: Schema.optionalKey(Schema.Struct({ enabled: Schema.optionalKey(Schema.Boolean) })),
    }),
  ),
});
  ...
  return {
    ...
    providers: {
      ...
      amp: {
        ...
        enabled: persisted.providers?.amp?.enabled ?? usedProviders.has("amp"),
      },
    },
    providerInstances,
  };
...
const PERSISTED_SERVER_SETTINGS_DEFAULTS = {
  ...
  providers: {
    ...
    amp: { ...DEFAULT_SERVER_SETTINGS.providers.amp, enabled: undefined },
  },
};
...
const make = Effect.gen(function* () {
  ...
  const loadSettingsFromDisk = Effect.gen(function* () {
    ...
    const providerHistory = yield* sql<{
      ...
      WHERE provider_name IN ('cursor', 'grok', 'opencode', 'amp')
      UNION
      SELECT DISTINCT
        provider_name AS "providerName",
        provider_instance_id AS "providerInstanceId"
